...
FLAGS = flags.FLAGS
# model & training
flags.DEFINE_integer('total_steps', 20000, "total number of training steps")
...
# privacy
flags.DEFINE_float('sigma_n', 1, "epsilon (the privacy budget)")
flags.DEFINE_float('clipping_bound', 1.0, "upper bound of the L2 norm of the loss gradients")
...
class Hinge(torch.nn.Module):
    def forward(self, pred_real, pred_fake=None):
        if pred_fake is not None:
            loss_real = torch.nn.functional.relu(1 - pred_real).mean()
            loss_fake = torch.nn.functional.relu(1 + pred_fake).mean()
            return loss_real + loss_fake
        else:
            loss = -pred_real.mean()
            return loss
...
def train():
    dataset = datasets.CIFAR10(
            ...
            transform=transforms.Compose([
                ...
                transforms.Lambda(lambda x: x + torch.rand_like(x) / 128) # instance noise
            ]))
    ...
    sigma_n_2 = 4 * (FLAGS.batch_size ** 2)
        ...
        for step in pbar:
            ...
            for _ in range(FLAGS.n_dis):
                ...
                for x in real:
                    ...
                    net_D_real = net_D(x)
                    loss_real = torch.nn.functional.relu(1 - net_D_real).mean()
                    loss_real.backward()
                    # clip sample grad
                    for param in net_D.parameters():
                        per_sample_grad = torch.clone(param.grad.detach())
                        pg_norm = torch.norm(per_sample_grad)
                        if pg_norm > FLAGS.clipping_bound:
                            per_sample_grad = per_sample_grad / pg_norm
                        param.accumulated_grads.append(per_sample_grad)
                # accumulate and add noise
                for param in net_D.parameters():
                    param.grad = sum(param.accumulated_grads)
                    torch.normal(mean=param.grad, std=FLAGS.sigma_n, out=param.grad)
                

                net_D_fake = net_D(fake)
                loss_fake = torch.nn.functional.relu(1 + net_D_fake).mean()
                loss_fake.backward()
                optim_D.step()
            # train the generator
            ...
            if step == 1 or step % FLAGS.eval_step == 0:
                ...
                if FLAGS.record:
                    ...
                    pbar.write(
                        ...
                        "FID Score: %6.3f" % (
                            step, FLAGS.total_steps, is_score[0], is_score[1], fid_score))
